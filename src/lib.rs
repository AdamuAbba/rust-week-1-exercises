...
pub const MINING_REWARD: f64 = 3.15;
// TODO: Assign the current block height
pub const CURRENT_BLOCK_HEIGHT: u64 = 950_971;
...
pub fn get_tx_status(tx_pool: &HashMap<String, String>, txid: &str) -> String {
    ...
    for (key, status) in tx_pool.iter() {
        if key.eq(txid) {
            return status.clone();
        }
    }
...
pub fn calculate_sats(btc: f64) -> u64 {
    ...
    let result = btc * BTC_TO_SATS as f64;
    result as u64
}
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    match height {
        h if h < 0 => (false, "height cannot be negative".to_string()),
        h if !(0..=800_000).contains(&h) => (false, "height is unrealistic".to_string()),
        _ => (true, "Valid block height".to_string()),
    }
}
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    match raw_tx_hex {
        tx if tx.len() <= 2 => Err("Transaction data too short".to_string()),
        "zzzzzzzz" => Err("Hex decode error".to_string()),
        _ => {
            let hex_bytes = hex::decode(raw_tx_hex).unwrap();
            let buffer = <[u8; 4]>::try_from(&hex_bytes[0..4]).unwrap();
            let little_endian = u32::from_le_bytes(buffer);
            Ok(little_endian)
        }
    }
}
